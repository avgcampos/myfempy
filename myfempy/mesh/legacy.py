...
def get_legacy_quad4(GEOMETRY):
    ...
    for i in range(1, nny+1, 1):
        coord[np.arange((i-1)*nnx, i*nnx, 1).tolist(),
              0] = np.arange((i-1)*nnx+1, i*nnx+1, 1).tolist()
        coord[np.arange((i-1)*nnx, i*nnx, 1).tolist(), 1] = xv[i-1, :]
        coord[np.arange((i-1)*nnx, i*nnx, 1).tolist(), 2] = yv[i-1, :]
    for i in range(1, nely+1, 1):
        conec[np.arange((i-1)*nelx, i*nelx, 1).tolist(),
              0] = np.arange((i-1)*nelx+1, i*nelx+1, 1).tolist()
        conec[np.arange((i-1)*nelx, i*nelx, 1).tolist(),
              1] = np.arange((i-1)*nnx+1, i*nnx, 1).tolist()
        conec[np.arange((i-1)*nelx, i*nelx, 1).tolist(),
              2] = np.arange((i-1)*nnx+2, i*nnx+1, 1).tolist()
        conec[np.arange((i-1)*nelx, i*nelx, 1).tolist(),
              3] = np.arange(i*nnx+2, (i+1)*nnx+1, 1).tolist()
        conec[np.arange((i-1)*nelx, i*nelx, 1).tolist(),
              4] = np.arange(i*nnx+1, (i+1)*nnx, 1).tolist()
    return conec, coord
