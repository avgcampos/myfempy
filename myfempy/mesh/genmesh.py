    ...
    def get_inci(elemlist, mat_lib, geo_lib, regions):
        ...
        for kk in range(nelem):
            ...
            if len(nodes) != MAXCONECELM:
                nodes = np.append(nodes, np.zeros(
                    int(MAXCONECELM-len(nodes))), axis=0)
            ...
            mesh_type_list[keyelem] = [elemset['id'], len(elemset['dofs']), len(
                elemset['nnodes']), len(elemset['tensor'])]
    ...
    def get_tabgeo(geolist):
        ...
        for gg in range(ngeo):
            ...
            if 'SEC' in geolist[gg].keys():
                ...
                t = geolist[gg]["DIM"]['t']
                d = geolist[gg]["DIM"]['d']
                A, Izz, Iyy, Jxx = sect_prop(
                    geolist[gg]["SEC"], geolist[gg]["DIM"])
    ...
    def get_data_mesh(meshdata):
        ...
        if 'LEGACY' in meshdata.keys():
            ...
            for ee in range(len(conec)):
                elem.append([int(conec[ee, 0]), elemtype, meshdata["PROPMAT"][0]["NAME"],
                            meshdata["PROPGEO"][0]["NAME"], conec[ee, 1:].astype(int).tolist()])

            elem = elem[1::][::]

            nodes = [[None]*4]
            for nn in range(len(coord)):
                nodes.append([int(coord[nn, 0]), coord[nn, 1],
                             coord[nn, 2], coord[nn, 3]])
            nodes = nodes[1::][::]
            ...
            if 'meshimport' in meshdata['GMSH'].keys():
                conec, nodes = convert_from_msh1(
                    os.getcwd()+'/'+meshdata['GMSH']['meshimport']['object'])
            else:

                get_gmsh_geo(meshdata)
                get_gmsh_msh(meshdata)
                conec, nodes = convert_from_msh1(
                    os.getcwd()+'/'+meshdata['GMSH']['filename'])
            ...
            for ee in range(len(conec)):
                if int(conec[ee][1]) == meshid:
                    contelm += 1
                    elem.append([contelm, meshdata['GMSH']['meshconfig']['elem'], meshdata["PROPMAT"][0]["NAME"],
                                meshdata["PROPGEO"][0]["NAME"], (np.array(conec[ee][5:])).astype(int).tolist()])
                ...
                if int(conec[ee][1]) == 15:
                    ...
                    if ptn1 != ptn0:
                        contptn += 1
                    reglist['reglist'].append({'type': 'point', 'list': str(
                        contptn), 'nodes': (np.array(conec[ee][5:])).astype(int).tolist()})
                    ptn0 = ptn1
                    ...
                    if edg1 != edg0:
                        contedg += 1
                    reglist['reglist'].append({'type': 'edge', 'list': str(
                        contedg), 'nodes': (np.array(conec[ee][5:])).astype(int).tolist()})
                    edg0 = edg1
                    ...
                    if sfr1 != sfr0:
                        contsfr += 1
                    reglist['reglist'].append({'type': 'surf', 'list': str(
                        contsfr), 'nodes': (np.array(conec[ee][5:])).astype(int).tolist()})
                    sfr0 = sfr1
            ...
            pointlist = {name.capitalize(): {key: [] for key in keys}
                         for name in list_point}

            list_edge = (np.arange(1, contedg+1).astype(str)).tolist()
            edgelist = {name.capitalize(): {key: [] for key in keys}
                        for name in list_edge}

            list_surf = (np.arange(1, contsfr+1).astype(str)).tolist()
            surflist = {name.capitalize(): {key: [] for key in keys}
                        for name in list_surf}
        ...
        if "ADD121" in meshdata.keys():
            if (mesh == 'legacyON') or (mesh == 'gmshON'):
                ...
                for nn in range(len(meshdata["NODELIST"])):
                    node = search_nodexyz(
                        meshdata["NODELIST"][nn][1], meshdata["NODELIST"][nn][2], meshdata["NODELIST"][nn][3], coord, 2E-3)
                    if len(node) == 0:
                        nodefind.extend([len(coord)+1])
                        coord = np.append(coord, np.array(
                            [[len(coord)+1, meshdata["NODELIST"][nn][1], meshdata["NODELIST"][nn][2], meshdata["NODELIST"][nn][3]]]), axis=0)
                    else:
        ...
        inci, mesh_type_list = MeshSet.get_inci(
            elemlist, mat_lib, geo_lib, regions)
