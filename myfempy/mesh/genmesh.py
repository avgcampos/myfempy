#!/usr/bin/env python
from myfempy.utils.path import create_user_path
            ...
            for ee in range(len(conec)):
                elem.append(
                    [
                        int(conec[ee, 0]),
                        elemtype,
                        meshdata["PROPMAT"][0]["NAME"],
                        meshdata["PROPGEO"][0]["NAME"],
        ...
        if "ADD121" in meshdata.keys():
            if (mesh == "legacyON") or (mesh == "gmshON"):
                ...
                for nn in range(len(meshdata["NODELIST"])):
                    node = search_nodexyz(
                        meshdata["NODELIST"][nn][1],
                        meshdata["NODELIST"][nn][2],
                        meshdata["NODELIST"][nn][3],
                        coord,
                        2e-3,
                    )
                    if len(node) == 0:
                ...
                elemlist.append(
                    [
                        len(elemlist),
                        meshdata["ELEMLIST"][0][1],
                        meshdata["ELEMLIST"][0][2],
                        meshdata["ELEMLIST"][0][3],
                        nodefind,
                    ]
                )
            else:
