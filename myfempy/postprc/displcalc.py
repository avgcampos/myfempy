...
class Deformation:
    ...
    def ux(self, U):
        ...
        for nn in range(1, self.nnode+1):
            ...
            Umag[nn-1, 0] = np.sqrt(U[self.nodedof*nn-2]
                                    ** 2 + U[self.nodedof*nn-1]**2)

        # meshDefU = np.concatenate((self.coord[:, 0][:, np.newaxis], np.add(
        #     self.coord[:, 1:], Udef)), axis=1)
    ...
    def uy_rz(self, U):
        ...
        # meshDefU = np.concatenate((self.coord[:, 0][:, np.newaxis], np.add(
        #     self.coord[:, 1:], scale*Udef)), axis=1)
    ...
    def ux_uy_rz(self, U):
        ...
        for nn in range(1, self.nnode+1):
            ...
            Udef[nn-1, 1] = U[self.nodedof*nn-2]
            # Rdef[nn-1,2] = U[self.nodedof*nn-1,0]
            Umag[nn-1, 0] = np.sqrt(U[self.nodedof*nn-3]
                                    ** 2 + U[self.nodedof*nn-2]**2)
    ...
    def ux_uy_uz_rx_ry_rz(self, U):
        ...
        for nn in range(1, self.nnode+1):
            ...
            # Rdef[nn-1,2] = U[self.nodedof*nn-1,0]
            Umag[nn-1, 0] = np.sqrt(U[self.nodedof*nn-6]**2 +
                                    U[self.nodedof*nn-5]**2 + U[self.nodedof*nn-4]**2)
    ...
    def ux_uy_uz(self, U):
        ...
        for nn in range(1, self.nnode+1):
            ...
            Umag[nn-1, 0] = np.sqrt(U[self.nodedof*nn-3]**2 +
                                    U[self.nodedof*nn-2]**2 + U[self.nodedof*nn-1]**2)

        # meshDefU = np.concatenate((self.coord[:,0][:, np.newaxis],np.add(self.coord[:,1:],Udef)),axis=1)
