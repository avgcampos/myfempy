...
class PostComputer:
    ...
    def save_vtk(self, postporc_result, postprocset):
        ...
        if 'displ' in postprocset['COMPUTER'].keys():
            for st in range(len(postporc_result['displ'])):
                ...
                # (postprocset["PLOTSET"]['filename']+'_'+str(st+1)+'_'+postporc_result['solution'][pp]['name'])
                plotdata['filename'] = (
    ...
    def main(self, postprocset):
        ...
        # scale = (postprocset["SCALE"]/100) * \
        #     (1/(max(abs(postprocset["SOLUTION"][:, 0]))))

        result_disp = np.zeros((postprocset["SOLUTION"].shape[1], self.modelinfo['coord'].shape[0], 4))
        # meshdefU = np.zeros(
        #     (postprocset["SOLUTION"].shape[1], self.modelinfo['coord'].shape[0], self.modelinfo['coord'].shape[1]))
        for ns in range(postprocset["SOLUTION"].shape[1]):
            result_disp[ns, :, :] = PostComputer.displ(self, postprocset["SOLUTION"][:, ns])
        ...
        if 'frf' in postprocset['COMPUTER'].keys():
            if postprocset['COMPUTER']['frf'] == True:
                postporc_result['frf'].append({'val': postprocset["SOLUTION"], 'freqlog': postprocset["FREQ"]})
            else:
                pass
