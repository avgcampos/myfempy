...
class Plane(Element):
    ...
    def getStifLinearMat(Model, inci, coord, tabmat, tabgeo, intgauss, element_number):
        ...
        for pp in range(intgauss):
            detJ = Model.shape.getdetJacobi(pt[pp], elementcoord)             #critico          
            diffN = Model.shape.getDiffShapeFuntion(pt[pp], nodedof)          #critico         
            invJ = Model.shape.getinvJacobi(pt[pp], elementcoord, nodedof)    #critico         
            BCB = BTCB(diffN, H, invJ, C)                                     #critico         
