    ...
    with open(file_input, 'r') as file_object:
        ...
        if line == '#SOLVER_CFG\n':
            ...
            while line != "\n":
                lineaux = line.split()
                linearray = np.array(
                    [lineaux[0], lineaux[2], lineaux[3], lineaux[4], lineaux[5], lineaux[6], lineaux[7]])
        ...
        if line == '#PROP_GEOMETRIA\n':
            ...
            if line == '##GEO_INP\n':
                ...
                while line != "\n":
                    lineaux = line.split()
                    linearray = np.array(
                        [lineaux[0], lineaux[2], lineaux[3], lineaux[4], lineaux[5], lineaux[6]])
                    propgeonewlist = np.append(
                        propgeonewlist, [linearray], axis=0)
                    line = file_object.readline()
            line = file_object.readline()
            if line == '##GEO_BIB\n':
                line = file_object.readline()
                while line != "\n":
                    ...
                    linearray = np.array(
                        [lineaux[0], lineaux[2], lineaux[3], lineaux[4], lineaux[5], lineaux[6]])
                    propgeobiblist = np.append(
                        propgeobiblist, [linearray], axis=0)
                    line = file_object.readline()
        line = file_object.readline()
        ...
        if line == '##MAT_CFG\n':
            ...
            while line != "\n":
                lineaux = line.split()
                linearray = np.array([lineaux[0], lineaux[2], lineaux[3],
                                     lineaux[4], lineaux[5], lineaux[6], lineaux[7], lineaux[8]])
            ...
            if line == '##LOADS\n':
                ...
                while line != "\n":
                    ...
                    linearray = np.array([lineaux[0], lineaux[2], lineaux[3], lineaux[4],
                                         lineaux[5], lineaux[6], lineaux[7], lineaux[8], lineaux[9]])
        ...
        if line == '#GRAPH_OUT\n':
            ...
            while line != "\n":
                ...
                graphout = np.append(
                    graphout, np.array([[lineaux[0]]]), axis=1)
...
def gen_usrlog(usrlog_file, solutconfig, meshconfig, compmaterial, forcelist, boncdlist, solvercfg, outputcfg, graphout, fileout):
    with open(usrlog_file, 'w') as file_object:
        ...
        for j in range(len(np.unique(forcelist[:, 8]))):
            forceliststep = forcelist[np.where(
                forcelist[:, 8] == str(j+1)), :][0]
            for i in range(len(forceliststep)):
                file_object.write('force_typ_'+str(i)+' ''= ' +
                                  "'"+forceliststep[i, 1]+"'"+'\n')
                file_object.write('force_opt_'+str(i)+' ''= ' +
                                  "'"+forceliststep[i, 4]+"'"+'\n')
                file_object.write('force_opt_dir'+str(i) +
                                  ' ''= '+"'"+forceliststep[i, 2]+"'"+'\n')
                file_object.write('force_step'+str(j)+' ''= ' +
                                  "'"+forceliststep[i, 8]+"'"+'\n')
        for i in range(len(boncdlist)):
            file_object.write('bc_typ_'+str(i)+' ''= ' +
                              "'"+boncdlist[i, 1]+"'"+'\n')
            file_object.write('bc_opt_'+str(i)+' ''= ' +
                              "'"+boncdlist[i, 3]+"'"+'\n')
            file_object.write('bc_opt_dir'+str(i)+' ''= ' +
                              "'"+boncdlist[i, 2]+"'"+'\n')
...
def import_mesh_line(file_imp):
    with open(file_imp, 'r') as file_object:
        ...
        for ii in range(0, NNOD):
            ...
            lineaux = line.split()
            contstr = lineaux[0:4]
            coord[ii, :] = [float(contstr[0]), float(
        ...
        for kk in range(0, NELM):
            ...
            if type_elm == 1:
                contelm += 1
                prop_elm[contelm-1, :] = [type_elm,
                                          float(lineaux[3]), float(lineaux[3])]
                conec_elm[contelm-1, :] = [contelm,
                                           float(lineaux[5]), float(lineaux[6]), 0, 0, 0, 0, 0, 0]
    ...
    with open(file_imp, 'r') as file_object:
        ...
        for kk in range(0, NELM):
            ...
            if type_elm == 2:
                ...
                prop_elm[contelm-1, :] = [type_elm,
                                          float(lineaux[3]), float(lineaux[3])]
                conec_elm[contelm-1, :] = [contelm, float(lineaux[5]), float(
                    lineaux[6]), float(lineaux[7]), 0, 0, 0, 0, 0]
    conec_elm = conec_elm[:contelm, :]
    prop_elm = prop_elm[:contelm, :]
    inci = np.concatenate(
        (conec_elm[:, 0][:, np.newaxis], prop_elm, conec_elm[:, 1:]), axis=1)
        ...
        for kk in range(0, NELM):
            ...
            if type_elm == 3:
                ...
                prop_elm[contelm-1, :] = [type_elm,
                                          float(lineaux[3]), float(lineaux[3])]
                conec_elm[contelm-1, :] = [contelm, float(lineaux[5]), float(
                    lineaux[6]), float(lineaux[7]), float(lineaux[8]), 0, 0, 0, 0]
    conec_elm = conec_elm[:contelm, :]
    prop_elm = prop_elm[:contelm, :]
    inci = np.concatenate(
        (conec_elm[:, 0][:, np.newaxis], prop_elm, conec_elm[:, 1:]), axis=1)
    ...
    with open(file_imp, 'r') as file_object:
        ...
        for kk in range(0, NELM):
            ...
            if type_elm == 5:
                ...
                prop_elm[contelm-1, :] = [type_elm,
                                          float(lineaux[3]), float(lineaux[3])]
                conec_elm[contelm-1, :] = [contelm, float(lineaux[5]), float(lineaux[6]), float(lineaux[7]), float(
                    lineaux[8]), float(lineaux[9]), float(lineaux[10]), float(lineaux[11]), float(lineaux[12])]
    conec_elm = conec_elm[:contelm, :]
    prop_elm = prop_elm[:contelm, :]
    inci = np.concatenate(
        (conec_elm[:, 0][:, np.newaxis], prop_elm, conec_elm[:, 1:]), axis=1)
