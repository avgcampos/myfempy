    ...
    with open(log_file, "w") as file_object:
        ...
        if 'get' in log_data.keys():
            ...
            if 'inci' in log_data['get'].keys():
                ...
                file_object.write('{0:<7}{1:<7}{2:<7}{3:<7}{4:<25}\n'.format('ELEM','KEY','MAT','GEO','NODES'))
                for row in range(len(modelinfo["inci"])):
                    node_list = modelinfo["inci"][row][4:].astype(int) # node_list[np.nonzero(node_list)].tolist()
                    node_list = node_list[np.nonzero(node_list)].tolist()
                    node_list = " ".join(str(x) for x in node_list)
                    file_object.write('{0:<7}{1:<7}{2:<7}{3:<7}{4:<25}\n'.format(modelinfo["inci"][row][0].astype(int),modelinfo["inci"][row][1].astype(int),modelinfo["inci"][row][2].astype(int),modelinfo["inci"][row][3].astype(int),node_list))
            if 'coord' in log_data['get'].keys():
                file_object.write("\n")
                file_object.write("LIST OF NODES COORDINATE\n")
                file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}\n'.format('NODE','X','Y','Z'))
                for row in range(len(modelinfo["coord"])):
                    file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}\n'.format(modelinfo["coord"][row][0].astype(int),modelinfo["coord"][row][1],modelinfo["coord"][row][2],modelinfo["coord"][row][3]))
            if 'boundcond_list' in log_data['get'].keys():
                ...
                for row in range(len(modelinfo["constrains"])):
                    ...
                    if bc_type==0:
                        file_object.write('{0:<7}{1:<10}\n'.format('ALL',modelinfo["constrains"][row][1].astype(int)))
                    elif bc_type==1:
                        file_object.write('{0:<7}{1:<10}\n'.format('UX',modelinfo["constrains"][row][1].astype(int)))
                    elif bc_type==2:
                        file_object.write('{0:<7}{1:<10}\n'.format('UY',modelinfo["constrains"][row][1].astype(int)))
                    elif bc_type==3:
                        file_object.write('{0:<7}{1:<10}\n'.format('UZ',modelinfo["constrains"][row][1].astype(int)))
                    elif bc_type==4:
            ...
            if 'forces_list' in log_data['get'].keys():
                ...
                file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format('NODE','TYPE','VALUE','STEP'))
                for row in range(len(modelinfo["forces"])):
                    fc_type = modelinfo["forces"][row][1]
                    if fc_type ==1:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'FX',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    elif fc_type ==2:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'FY',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    elif fc_type ==3:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'FZ',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    elif fc_type ==4:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'TX',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    elif fc_type ==5:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'TY',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    elif fc_type ==6:
                        file_object.write('{0:<7}{1:<10}{2:<25}{3:<10}\n'.format(modelinfo["forces"][row][0].astype(int),'TZ',modelinfo["forces"][row][2],modelinfo["forces"][row][3].astype(int)))
                    else:
                        pass
            if 'tabmat' in log_data['get'].keys():
                file_object.write("\n")
                file_object.write("LIST OF MATERIAL PROPERTY\n")
                file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}{4:<10}{5:<10}{6:<10}{7:<10}{8:<10}{9:<10}{10:<10}{11:<10}\n'.format('PROP','EXX','VXX','GXX','EYY','VYY','GYY','RHO','STIF','DAMP','ID_MAT','ID_DEF')) 
                for row in range(len(modelinfo["tabmat"])):
                    file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}{4:<10}{5:<10}{6:<10}{7:<10}{8:<10}{9:<10}{10:<10}{11:<10}\n'.format(str(row+1),modelinfo["tabmat"][row][0],
                                                                                                                     modelinfo["tabmat"][row][1],modelinfo["tabmat"][row][2],
                                                                                                                     modelinfo["tabmat"][row][3],modelinfo["tabmat"][row][4],
                                                                                                                     modelinfo["tabmat"][row][5],modelinfo["tabmat"][row][6],
                                                                                                                     modelinfo["tabmat"][row][7],modelinfo["tabmat"][row][8],
                                                                                                                     modelinfo["tabmat"][row][9].astype(int),modelinfo["tabmat"][row][10].astype(int))) 
            if 'tabgeo' in log_data['get'].keys():
                file_object.write("\n")
                file_object.write("LIST OF MATERIAL PROPERTY\n")
                file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}{4:<10}{5:<10}{6:<10}{7:<10}{8:<10}{9:<10}{10:<10}\n'.format('PROP','AREA','IZZ','IYY','JXX','THK','B','H','T','D','ID_SEC')) 
                for row in range(len(modelinfo["tabgeo"])):
                    file_object.write('{0:<7}{1:<10}{2:<10}{3:<10}{4:<10}{5:<10}{6:<10}{7:<10}{8:<10}{9:<10}{10:<10}\n'.format(str(row+1),modelinfo["tabgeo"][row][0],
                                                                                                                     modelinfo["tabgeo"][row][1],modelinfo["tabgeo"][row][2],
                                                                                                                     modelinfo["tabgeo"][row][3],modelinfo["tabgeo"][row][4],
                                                                                                                     modelinfo["tabgeo"][row][5],modelinfo["tabgeo"][row][6],
                                                                                                                     modelinfo["tabgeo"][row][7],modelinfo["tabgeo"][row][8],
                                                                                                                     modelinfo["tabgeo"][row][9].astype(int)))     
        #=============================================================================================================
        file_object.write("\n")
        file_object.write("+------------------S O L V E R   S T A T U S------------------+\n")
        file_object.write('{0:<30} : {1:<10} SEC\n'.format("ASSEMBLY FULL TIME SPEND ",str(solstatus['timeasb'])))
        file_object.write('{0:<30} : {1:<10} SEC\n'.format("SOLVE FULL TIME SPEND ",str(solstatus['timesim'])))
        file_object.write('{0:<30} : {1:<10} MB\n'.format("STIFFNESS SIZE ",str(solstatus['kgsize'])))
