...
def convert_to_vtk(plotdata):
    with open(plotdata['filename']+'.vtk', 'w') as file_object:
        ...
        file_object.write(
            'POINTS '+str(int(len(plotdata['coord'])))+' double\n')
        ...
        file_object.write('\n')
        file_object.write('CELLS'+' '+str(int(len(plotdata['inci'])))+' '+str(
            (max(plotdata['nodecon'])+1)*int(len(plotdata['inci'])))+'\n')
        for ii in range(0, int(len(plotdata['inci']))):
            listnodes = plotdata['inci'][ii, 4:]
            list2write = np.array(list(map(
                lambda x: x-1, listnodes[listnodes.nonzero()]))).astype(int).astype(str).tolist()
            file_object.write(str(len(list2write))+' ' +
                              ' '.join(list2write)+'\n')
        file_object.write('\n')
        file_object.write('CELL_TYPES'+' ' +
        ...
        if len(plotdata['stress_POINT_DATA_val']) > 0:
            for jj in range(0, int(len(plotdata['stress_POINT_DATA_title']))):
                ...
                list2write = plotdata['stress_POINT_DATA_val'][:, jj].astype(
                    str).tolist()
                file_object.write('\n'.join(list2write)+'\n')
                file_object.write('\n')
        file_object.write('\n')
        if len(plotdata['displ_POINT_DATA_val']) > 0:
            ...
            file_object.write(plotdata['displ_POINT_DATA_title']+' 3 ' +
                              str(int(len(plotdata['displ_POINT_DATA_val'])))+' float\n')
            for ii in range(0, int(len(plotdata['displ_POINT_DATA_val']))):
                list2write = plotdata['displ_POINT_DATA_val'][ii, :].astype(
                    str).tolist()
                file_object.write(' '.join(list2write)+'\n')
        if len(plotdata['modes_POINT_DATA']) > 0:
            for md in range(0, int(len(plotdata['modes_POINT_DATA']))):
                file_object.write('\n')
                file_object.write('FIELD FieldData 1\n')
                file_object.write(plotdata['modes_POINT_DATA'][md]['title']+' 3 '+str(
                    int(len(plotdata['modes_POINT_DATA'][md]['val'][:, 1:])))+' float\n')
                for ii in range(0, int(len(plotdata['modes_POINT_DATA'][md]['val'][:, 1:]))):
                    list2write = plotdata['modes_POINT_DATA'][md]['val'][:, 1:][ii, :].astype(
                        str).tolist()
                    file_object.write(' '.join(list2write)+'\n')
        file_object.write('\n')
        ...
        if len(plotdata['stress_CELL_DATA_val']) > 0:
            for jj in range(0, int(len(plotdata['stress_CELL_DATA_title']))):
                ...
                file_object.write('LOOKUP_TABLE default\n')
                list2write = plotdata['stress_CELL_DATA_val'][:, jj].astype(
                    str).tolist()
                file_object.write('\n'.join(list2write)+'\n')
...
def convert_from_vtk(filename):
    ...
    with open(file_imp, 'r') as file_object:
        ...
        for ii in range(0, NNOD):
            ...
            lineaux = line.split()
            contstr = lineaux[0:3]
            nodelist.append([int(ii+1), float(contstr[0]),
                            float(contstr[1]), float(contstr[2])])
        nodelist = nodelist[1::][::]
        file_object.readline()
        line = file_object.readline()
        lineaux = line.split()
