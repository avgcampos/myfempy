...
class Tensor:
    ...
    def strain(self):
        ...
        strn_elm_eqv = np.sqrt(0.5*((epsilon[0]-epsilon[1])**2 + (epsilon[1]-epsilon[2])**2 + (epsilon[2]-epsilon[0])**2 + 6*(epsilon[3]**2 + epsilon[4]**2 + epsilon[5]**2))) 
            
        strain = [strn_elm_eqv,strn_elm_xx,strn_elm_yy,strn_elm_zz,strn_elm_xy,strn_elm_yz,strn_elm_zx]
        
        title =  ["STRAIN_VM", "STRAIN_XX", "STRAIN_YY", "STRAIN_ZZ", "STRAIN_XY", "STRAIN_YZ", "STRAIN_ZX"]
    ...
    def stress(self, epsilon):
        ...
        strs_elm_eqv = np.sqrt(0.5*((sigma[0]-sigma[1])**2 + (sigma[1]-sigma[2])**2 + (sigma[2]-sigma[0])**2 + 6*(sigma[3]**2 + sigma[4]**2 + sigma[5]**2))) 

        stress = [strs_elm_eqv,strs_elm_xx,strs_elm_yy,strs_elm_zz,strs_elm_xy,strs_elm_yz,strs_elm_zx]
        
        title =  ["STRESS_VM", "STRESS_XX", "STRESS_YY", "STRESS_ZZ", "STRESS_XY", "STRESS_YZ", "STRESS_ZX"]
