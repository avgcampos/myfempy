...
class Tetra4(Shape):
    ...
    def getDiffShapeFuntion(shape_function, r_coord, nodedof):
        ...
        for block in range(4):
            for dof in range(nodedof):
                mat_diff_N[nodedof*dof-dof*(nodedof-3), block*nodedof+dof] = diff_shape_function[0, block]
                mat_diff_N[nodedof*dof-dof*(nodedof-3)+1, block*nodedof+dof] = diff_shape_function[1, block]
                mat_diff_N[nodedof*dof-dof*(nodedof-3)+2, block*nodedof+dof] = diff_shape_function[2, block]
        return mat_diff_N
    ...
    def getShapeKey(nodelist, nodedof):
        ...
        for node in range(len(nodelist)):
            for dof in range(nodedof):
                shape_key[nodedof*node+dof] = nodedof * nodelist[node] - (nodedof-dof)
