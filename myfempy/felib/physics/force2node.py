...
def force_edge(modelinfo, force_value, force_dirc, node_list_fc, dir_fc):
    ...
    for ii in range(len(node_list_fc)):
        elm2list = modelinfo["inci"][
            (np.asarray(np.where(modelinfo["inci"][:, 4:] == node_list_fc[ii])))[0][:],
            0,
        ]
    ...
    if modelinfo["elemid"][0] == 210:
        ...
        for i in range(len(elmlist)):
            ...
            if np.any([noi == node_list_fc[:]]):
                ...
                continue
            L = np.sqrt(
                (modelinfo["coord"][no1 - 1, 1] - modelinfo["coord"][no2 - 1, 1]) ** 2
                + (modelinfo["coord"][no1 - 1, 2] - modelinfo["coord"][no2 - 1, 2]) ** 2
            ...
            loc = np.array(
                [
                    modelinfo["nodedof"][0] * no1dof,
                    modelinfo["nodedof"][0] * no1dof + 1,
                    modelinfo["nodedof"][0] * no2dof,
                    modelinfo["nodedof"][0] * no2dof + 1,
                ]
            )
            tck = modelinfo["tabgeo"][
                int(modelinfo["inci"][int(elmlist[i] - 1), 3] - 1), 4
            ]
            if force_dirc == "fx":
        ...
        for i in range(len(elmlist)):
            ...
            loc = np.array(
                [
                    modelinfo["nodedof"][0] * no1dof,
                    modelinfo["nodedof"][0] * no1dof + 1,
                    modelinfo["nodedof"][0] * no1dof + 2,
                    modelinfo["nodedof"][0] * no2dof,
                    modelinfo["nodedof"][0] * no2dof + 1,
                    modelinfo["nodedof"][0] * no2dof + 2,
                    modelinfo["nodedof"][0] * no3dof,
                    modelinfo["nodedof"][0] * no3dof + 1,
                    modelinfo["nodedof"][0] * no3dof + 2,
                    modelinfo["nodedof"][0] * no4dof,
                    modelinfo["nodedof"][0] * no4dof + 1,
                    modelinfo["nodedof"][0] * no4dof + 2,
                ]
            )
            if force_dirc == "fx":
                force_value_vector[loc, 0] += np.array(
        ...
        for i in range(len(elmlist)):
            ...
            if no1dof.size != 0:
                if no2dof.size != 0:
                    L = np.sqrt(
                        (
                            modelinfo["coord"][no1 - 1, 1]
                            - modelinfo["coord"][no2 - 1, 1]
                        )
                        ** 2
                        + (
                            modelinfo["coord"][no1 - 1, 2]
                            - modelinfo["coord"][no2 - 1, 2]
                        )
                        ** 2
                        + (
                            modelinfo["coord"][no1 - 1, 3]
                            - modelinfo["coord"][no2 - 1, 3]
                        )
                        ** 2
                    )
                    ...
                    loc = np.array(
                        [
                            modelinfo["nodedof"][0] * no1dof[0][0],
                            modelinfo["nodedof"][0] * no1dof[0][0] + 1,
                            modelinfo["nodedof"][0] * no2dof[0][0],
                            modelinfo["nodedof"][0] * no2dof[0][0] + 1,
                        ]
                    )
                    force_value_vector[loc, 0] += np.array(
                        [
                ...
                if no2dof.size != 0:
                    ...
                    loc = np.array(
                        [
                            modelinfo["nodedof"][0] * no1dof[0][0],
                            modelinfo["nodedof"][0] * no1dof[0][0] + 1,
                            modelinfo["nodedof"][0] * no1dof[0][0] + 2,
                            modelinfo["nodedof"][0] * no2dof[0][0],
                            modelinfo["nodedof"][0] * no2dof[0][0] + 1,
                            modelinfo["nodedof"][0] * no2dof[0][0] + 2,
                        ]
                    )
                    if force_dirc == "fx":
                        force_value_vector[loc, 0] += np.array(
                ...
                if no2dof.size != 0:
                    ...
                    loc = np.array(
                        [
                            ...
                            modelinfo["nodedof"][0] * no1dof[0][0] + 1,
                            modelinfo["nodedof"][0] * no1dof[0][0] + 2,
                            modelinfo["nodedof"][0] * no1dof[0][0] + 3,
                            modelinfo["nodedof"][0] * no1dof[0][0] + 4,
                            modelinfo["nodedof"][0] * no1dof[0][0] + 5,
                            modelinfo["nodedof"][0] * no2dof[0][0],
                            modelinfo["nodedof"][0] * no2dof[0][0] + 1,
                            modelinfo["nodedof"][0] * no2dof[0][0] + 2,
                            modelinfo["nodedof"][0] * no2dof[0][0] + 3,
                            modelinfo["nodedof"][0] * no2dof[0][0] + 4,
                            modelinfo["nodedof"][0] * no2dof[0][0] + 5,
                        ]
                    )
                    if force_dirc == "fx":
                        force_value_vector[loc, 0] += np.array(
