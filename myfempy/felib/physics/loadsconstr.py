    ...
    for k in range(int(len(np.unique([flist[:, 7]])))):
        ...
        for i in range(len(forcelist)):
            ...
            if forcelist[i][3] == "lenghx":
                ...
                node_list_fc = modelinfo['coord'][np.where(
                    (modelinfo['coord'][:, 1] >= coord_0) & (modelinfo['coord'][:, 1] <= coord_1)), 0][0]
                dir_fc = 'x'
            elif forcelist[i][3] == "lengthy":
                coord_0 = float(forcelist[i, 5])
                coord_1 = float(forcelist[i, 6])
                node_list_fc = modelinfo['coord'][np.where(
                    (modelinfo['coord'][:, 2] >= coord_0) & (modelinfo['coord'][:, 2] <= coord_1)), 0][0]
                dir_fc = 'y'
            elif forcelist[i][3] == "lengthz":
                coord_0 = float(forcelist[i, 5])
                coord_1 = float(forcelist[i, 6])
                node_list_fc = modelinfo['coord'][np.where(
                    (modelinfo['coord'][:, 3] >= coord_0) & (modelinfo['coord'][:, 3] <= coord_1)), 0][0]
                dir_fc = 'z'
                ...
                if float(forcelist[i, 5]) == 999:
                    dir_fc = 'x_y'
                    coord_fc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 3] == float(forcelist[i, 6])), :])[0]
                elif float(forcelist[i, 6]) == 999:
                    dir_fc = 'x_z'
                    coord_fc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 2] == float(forcelist[i, 5])), :])[0]
                ...
                if float(forcelist[i, 4]) == 999:
                    ...
                    dir_fc = 'y_z'
                    coord_fc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 1] == float(forcelist[i, 4])), :])[0]
            ...
            # ----- SEEKERS IN TAGS -----
                ...
                if forcelist[i][1] == 'fx':
                    ...
                    pass
                node_list_fc = modelinfo['regions'][0][1][int(
                    forcelist[i][8])-1][1][:]
                ...
                if forcelist[i][1] == 'fx':
                    ...
                    pass
                node_list_fc = modelinfo['regions'][1][1][int(
                    forcelist[i][8])-1][1][:]
                ...
                if forcelist[i][1] == 'fx':
                    ...
                    pass
                node_list_fc = modelinfo['regions'][2][1][int(
                    forcelist[i][8])-1][1][:]
            else:
                print("input erro: force_opt don't defined")
            # ----- TYPE OF LOADS -----
            if forcelist[i][0] == "forcenode":
                force_value_vector = np.ones_like(
                    node_list_fc)*float(forcelist[i, 2])
                if forcelist[i][1] == 'fx':
                ...
                for j in range(len(node_list_fc)):
                    fcdef = np.array(
                        [[int(node_list_fc[j]), fc_type_dof[j], force_value_vector[j], int(forcelist[i][7])]])
                ...
                node_list_fc = np.repeat(
                    node_list_fc, modelinfo['nodedof'][0], axis=0)
                for j in range(len(node_list_fc)):
                    fcdef = np.array(
                        [[int(node_list_fc[j]), fc_type_dof[j], force_value_vector[j, 0], int(forcelist[i][7])]])
                    forcenodeaply = np.append(forcenodeaply, fcdef, axis=0)
        ...
        if blist[i][0] == "fixed":
            if blist[i][2] == "edgex":
                ...
                if float(blist[i, 4]) == 999:
                    coord_bc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 3] == float(blist[i, 5])), :])[0]
                elif float(blist[i, 5]) == 999:
                    coord_bc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 2] == float(blist[i, 4])), :])[0]
                ...
                if float(blist[i, 3]) == 999:
                    ...
                    coord_bc = (modelinfo['coord'][np.where(
                        modelinfo['coord'][:, 1] == float(blist[i, 3])), :])[0]
                ...
                node_list_bc = modelinfo['regions'][0][1][int(
                    blist[i][6])-1][1][:]
            elif blist[i][2] == "edge":
                node_list_bc = modelinfo['regions'][1][1][int(
                    blist[i][6])-1][1][:]
            elif blist[i][2] == "surf":
                node_list_bc = modelinfo['regions'][2][1][int(
                    blist[i][6])-1][1][:]
            else:
