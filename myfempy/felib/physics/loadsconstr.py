    ...
    for k in range(int(len(np.unique([flist[:,7]])))):
        ...
        for i in range(len(forcelist)):
            ...
            if forcelist[i][3] == "lenghx":
                ...
                node_list_fc = modelinfo['coord'][np.where((modelinfo['coord'][:,1]>=coord_0)&(modelinfo['coord'][:,1]<=coord_1)),0][0]
                dir_fc = 'x'
            
            elif forcelist[i][3] == "lengthy":
                coord_0 = float(forcelist[i,5])
                coord_1 = float(forcelist[i,6])
                node_list_fc = modelinfo['coord'][np.where((modelinfo['coord'][:,2]>=coord_0)&(modelinfo['coord'][:,2]<=coord_1)),0][0]
                dir_fc = 'y'
            
            elif forcelist[i][3] == "lengthz":
                coord_0 = float(forcelist[i,5])
                coord_1 = float(forcelist[i,6])
                node_list_fc = modelinfo['coord'][np.where((modelinfo['coord'][:,3]>=coord_0)&(modelinfo['coord'][:,3]<=coord_1)),0][0]
                dir_fc = 'z'
                ...
                if float(forcelist[i,5]) == 999:
                    dir_fc = 'x_y'
                    coord_fc=(modelinfo['coord'][np.where(modelinfo['coord'][:,3]==float(forcelist[i,6])),:])[0]
                elif float(forcelist[i,6]) == 999:
                    dir_fc = 'x_z'
                    coord_fc=(modelinfo['coord'][np.where(modelinfo['coord'][:,2]==float(forcelist[i,5])),:])[0]
                ...
                if float(forcelist[i,4]) == 999:
                    ...
                    dir_fc = 'y_z'
                    coord_fc=(modelinfo['coord'][np.where(modelinfo['coord'][:,1]==float(forcelist[i,4])),:])[0]
            ...
            #----- SEEKERS IN TAGS -----
                ...
                node_list_fc = search_nodexyz(node_coordX,node_coordY,node_coordZ,modelinfo['coord'],2E-3)
            ...
            if forcelist[i][0] == "forcenode":
                ...
                for j in range(len(node_list_fc)):
                    fcdef = np.array([[int(node_list_fc[j]) ,fc_type_dof[j], force_value_vector[j], int(forcelist[i][7])]])
                   ...
                   force_value_vector,fc_type_dof = force_beam(modelinfo,force_value,force_dirc,dir_fc,node_list_fc,line_fc)
                   ...
                   for j in range(len(node_list_fc)):
                       fcdef = np.array([[int(node_list_fc[j]),fc_type_dof[j],force_value_vector[j,0],int(forcelist[i][7])]])
                       forcenodeaply = np.append(forcenodeaply,fcdef,axis=0)                
                ...
                force_value_vector, fc_type_dof = force_edge(modelinfo,force_value,force_dirc,node_list_fc,dir_fc)
                ...
                force_value_vector,fc_type_dof = force_surf(modelinfo,force_value,force_dirc,node_list_fc,dir_fc)
        ...
        if blist[i][0] == "fixed":
            if blist[i][2] == "edgex":
                ...
                if float(blist[i,4]) == 999:
                    coord_bc=(modelinfo['coord'][np.where(modelinfo['coord'][:,3]==float(blist[i,5])),:])[0]
                elif float(blist[i,5]) == 999:
                    coord_bc=(modelinfo['coord'][np.where(modelinfo['coord'][:,2]==float(blist[i,4])),:])[0]
                ...
                if float(blist[i,3]) == 999:
                    ...
                    coord_bc=(modelinfo['coord'][np.where(modelinfo['coord'][:,1]==float(blist[i,3])),:])[0]
                ...
                node_list_bc = search_nodexyz(node_coordX,node_coordY,node_coordZ,modelinfo['coord'],2E-3)
