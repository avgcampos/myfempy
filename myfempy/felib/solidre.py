...
#------------------------------------------------------------------------------
# malha deformada plano 3D
def mesh_def_solid_3d(datamesh,coord,U,scale_mesh):
...
def stress_vm_plane_h8(U,datamesh,inci,coord,typeMechMat,tabmat,npp):  
...
def stress_avr_solid(strs_elm,datamesh,inci):
    nnode = datamesh[1]
    nel = datamesh[2]
    ith = np.zeros((8*nel),dtype=int)
    jth = np.zeros((8*nel),dtype=int)
    val = np.zeros((8*nel),dtype=int)
    ...
    # S = sp.csc_matrix(conecNode) 
   
    strs_avr = np.zeros((datamesh[1],1),dtype=float)
    for mm in range(datamesh[1]):
        # strs_avr[mm] = np.mean(strs_elm[np.unique(np.nonzero(S[mm,:])[0])])
