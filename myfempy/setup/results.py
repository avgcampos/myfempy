...
class setPostProcess(ABC):
    ...
    def getCompute(self, postprocset):
        ...
        if "structural" in  postprocset["COMPUTER"].keys():
            ...
            if "stress" in postprocset["COMPUTER"]["structural"].keys() and postprocset["COMPUTER"]["structural"]["stress"]==True:
    ...
    def getTracker(self, postprocset, postporc_result):
        ...
        for st in range(max_steps):
            ...
            val_X, val_Y, xlabel, ylabel = setPostProcess.__tracker_value(postporc_result, postprocset, plotset, self.modelinfo["coord"])
            hist_X.append(val_X)
            hist_Y.append(val_Y)
    ...
    def __tracker_value(postporc_result, postprocset, plotset, coord):
        ...
        if "point" in postprocset["TRACKER"].keys():
            ...
            val_Y = postporc_result['solution'][plotset["step"]]['val'][hist_node[0]-1, postprocset["TRACKER"]["point"]["dof"]]
            ...
            # hist_node = hist_node[0]-1
            # plotset["rstl"] = nodedof[0]*hist_node - (nodedof[0]-postprocset["TRACKER"]["data"]["displ"]["dof"])
            val_Y = postprocset["TRACKER"]['data']['y_data']     #plotset["val_list"][hist_node, postprocset["TRACKER"]["point"]["dof"]]
        ...
        #     ylabel = "DISPL MAG MIN"
            ...
            rstl = plotset["nodedof"] * (hist_node[0]-1) - (plotset["nodedof"] - postprocset["TRACKER"]["frf"]["dof"]-1)
            # val_Y = 20*np.log((abs(plotset["val_list"][rstl, :]))/10E-12)
            # val_Y = 20*np.log((abs(postporc_result['frf'][plotset["step"]]['val'][rstl, :]))/10E-12)
            val_Y = 20*np.log((abs(postporc_result['solution'][plotset["step"]]['val'][hist_node[0]-1, postprocset["TRACKER"]["frf"]["dof"]]))/10E-12)
