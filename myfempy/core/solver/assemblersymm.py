...
class AssemblerSYMM(Assembler):
    ...
    def getLinearStiffnessGlobalMatrixAssembler(Model, inci, coord, tabmat, tabgeo, intgauss, type_assembler, MP):
        ...
        for ee in range(inci.shape[0]):
            ...
            ith_diag, val_diag, ith_band, jth_band, val_band, nb, nd = AssemblerSYMM.__getVectorization(ith_band, jth_band, val_band, ith_diag, val_diag, nb, nd, loc, matrix, ee, elemdof)
    ...
    # @profile
    def __getVectorization(ith_band, jth_band, val_band, ith_diag, val_diag, nb, nd, loc, matrix, ee, elemdof):
        return getVectorizationSymm(ith_band, jth_band, val_band, ith_diag, val_diag, nb, nd, loc, matrix, ee, elemdof)
