...
# ==============================================================================
# NÚCLEO C INTERNO (LIVRE DE GIL E SEM EXCEÇÕES)
# ==============================================================================
...
@wraparound(False)         
@nonecheck(False)
cdef inline void C_getVectorization(
    INT32_t [::1] ith, 
    INT32_t [::1] jth, 
    FLT64_t [::1] val, 
    INT32_t [::1] loc, 
    ...
    cdef Py_ssize_t entries_per_elem = elemdof * elemdof
    cdef Py_ssize_t base_idx = entries_per_elem * ee
    cdef Py_ssize_t ii, jj, idx
    cdef INT32_t ki

    for ii in range(elemdof):
        ki = loc[ii]
        for jj in range(elemdof):
            idx = base_idx + ii * elemdof + jj
            ith[idx] = ki
            jth[idx] = loc[jj]
            val[idx] = matrix[ii, jj]

# ==============================================================================
# INTERFACE PYTHON (WRAPPER DEF)
# ==============================================================================
...
def getVectorization(
    ...
    C_getVectorization(ith, jth, val, loc, matrix, ee, elemdof)
    return ith, jth, val
