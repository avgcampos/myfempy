...
from numpy import array, zeros, transpose, dot, sqrt, concatenate, int32, float64
INT32 = int32
FLT64 = float64
...
class Plane(Element):
    ...
    def getB(Model, elementcoord, ptg, nodedof):
        ...
        L = Plane.getL()
        # B = dot(L, invJdiffN)
        # B = Plane.getL() @ invJ @ diffN
        return dot(L, invJdiffN)
    ...
    def getStifLinearMat(Model, inci, coord, tabmat, tabgeo, intgauss, element_number):
        ...
        for pp in range(intgauss):
            ...
            BT = transpose(B)
            BTC = dot(BT, C)
            K_elem_mat += dot(BTC, B)*L*detJ*wt[pp]*wt[pp]
    ...
    def getMassConsistentMat(Model, inci, coord, tabmat, tabgeo, intgauss, element_number):
        ...
        for pp in range(intgauss):
            ...
            M_elem_mat += dot(dot(transpose(matN), R), matN)*L*detJ*wt[pp]*wt[pp]
