...
@wraparound(False)         
@nonecheck(False)
cdef inline void MATN(FLT64 r0, FLT64 r1, FLT64 r2, FLT64 N[4]) noexcept nogil:
    N[0] = 1.0 - r0 - r1 - r2
    N[1] = r0
    N[2] = r1
    N[3] = r2
...
@wraparound(False)         
@nonecheck(False)
cdef inline void MATDIFFN(FLT64 r0, FLT64 r1, FLT64 r2, FLT64 dN[3][4]) noexcept nogil:
    dN[0][0] = -1.0
    dN[0][1] = 1.0
    dN[0][2] = 0.0
    dN[0][3] = 0.0
...
@wraparound(False)         
@nonecheck(False)
cdef inline void INV(FLT64 A[3][3], FLT64 invA[3][3]) noexcept nogil:
...
@wraparound(False)         
@nonecheck(False)
cdef inline void JACOBIANO(FLT64 r0, FLT64 r1, FLT64 r2, FLT64 [:, ::1] element_coord, FLT64 jac[3][3]) noexcept nogil:
    cdef FLT64 diffN[3][4]
    ...
    jac[0][2] = diffN[0][0] * element_coord[0, 2] + diffN[0][1] * element_coord[1, 2] + diffN[0][2] * element_coord[2, 2] + diffN[0][3] * element_coord[3, 2]
    ...
    jac[1][2] = diffN[1][0] * element_coord[0, 2] + diffN[1][1] * element_coord[1, 2] + diffN[1][2] * element_coord[2, 2] + diffN[1][3] * element_coord[3, 2]
    jac[2][0] = diffN[2][0] * element_coord[0, 0] + diffN[2][1] * element_coord[1, 0] + diffN[2][2] * element_coord[2, 0] + diffN[2][3] * element_coord[3, 0]
    jac[2][1] = diffN[2][0] * element_coord[0, 1] + diffN[2][1] * element_coord[1, 1] + diffN[2][2] * element_coord[2, 1] + diffN[2][3] * element_coord[3, 1]
    jac[2][2] = diffN[2][0] * element_coord[0, 2] + diffN[2][1] * element_coord[1, 2] + diffN[2][2] * element_coord[2, 2] + diffN[2][3] * element_coord[3, 2]
...
@wraparound(False)         
@nonecheck(False)
cdef inline void C_detJacobi(FLT64 r0, FLT64 r1, FLT64 r2, FLT64 [:, ::1] element_coord, FLT64 *detJ) noexcept nogil:
    cdef FLT64 jac[3][3]
    JACOBIANO(r0, r1, r2, element_coord, jac)
    cdef FLT64 raw_det = jac[0][0] * jac[1][1] * jac[2][2] + jac[0][1] * jac[1][2] * jac[2][0] + jac[0][2] * jac[1][0] * jac[2][1] - jac[2][0] * jac[1][1] * jac[0][2] - jac[2][1] * jac[1][2] * jac[0][0] - jac[2][2] * jac[1][0] * jac[0][1]
    detJ[0] = 0.166666666666667 * raw_det
...
@wraparound(False)         
@nonecheck(False)
cdef inline void C_ShapeFunctions(FLT64 r0, FLT64 r1, FLT64 r2, INT32 nodedof, FLT64[:, ::1] matN) noexcept nogil:
    cdef FLT64 N[4]
    MATN(r0, r1, r2, N)
    cdef Py_ssize_t block, dof
    for block in range(4):
        for dof in range(nodedof):
            # Correção: Acesso unidimensional correto ao array estático C N
...
@wraparound(False)         
@nonecheck(False)
cdef inline void C_DiffShapeFunction(FLT64 r0, FLT64 r1, FLT64 r2, INT32 nodedof, FLT64[:, ::1] matdiffN) noexcept nogil:
    cdef FLT64 diff_shape_function[3][4]
    MATDIFFN(r0, r1, r2, diff_shape_function)
    cdef Py_ssize_t block, dof
    for block in range(4):
        for dof in range(nodedof):
...
@wraparound(False)         
@nonecheck(False)
cdef inline void C_invJacobi(FLT64 r0, FLT64 r1, FLT64 r2, FLT64[:, ::1] element_coord, INT32 nodedof, FLT64[:, ::1] mat_invJ) noexcept nogil:
...
def DiffShapeFuntion(FLT64 [::1] point_gauss, INT32 nodedof):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] matdiffN = np.zeros((3 * nodedof, 4 * nodedof), dtype=np.float64) 
...
def StifLinear(
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] diffN = np.zeros((3 * nodedof, 4 * nodedof), dtype=np.float64)
