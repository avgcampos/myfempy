...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATN(FLT64 r0, FLT64 r1, FLT64 r2):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] N = np.zeros((1, 4), dtype=np.float64)
    N[0, 0] = 1.0 - r0 - r1 - r2
    N[0, 1] = r0
    N[0, 2] = r1
    N[0, 3] = r2
...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATDIFFN(FLT64 r0, FLT64 r1, FLT64 r2):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] dN = np.zeros((3, 4), dtype=np.float64)
    dN[0, 0] = -1.0
    dN[0, 1] = 1.0
    dN[1, 0] = -1.0
    dN[1, 2] = 1.0
...
def StifLinear(
    ...
    cdef np.ndarray[FLT64, ndim=1, mode="c"] current_pt_view = np.zeros(3, dtype=np.float64)
    ...
    for ip in range(intgauss):
        for jp in range(intgauss):
            for kp in range(intgauss):
                ...
                current_pt_view[2] = pt_gauss_nodes[kp]
