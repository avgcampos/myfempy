...
@wraparound(False)
@nonecheck(False)
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATN(FLT64 r0, FLT64 r1):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] N = np.zeros((1, 6), dtype=np.float64)
    N[0, 0] = (1.0 - r0 - r1) * (1.0 - 2.0 * r0 - 2.0 * r1)
    N[0, 1] = r0 * (2.0 * r0 - 1.0)
    N[0, 2] = r1 * (2.0 * r1 - 1.0)
    N[0, 3] = 4.0 * r0 * (1.0 - r0 - r1)
    N[0, 4] = 4.0 * r0 * r1
    N[0, 5] = 4.0 * r1 * (1.0 - r0 - r1)
    return N
...
@wraparound(False)
@nonecheck(False)
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATDIFFN(FLT64 r0, FLT64 r1):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] dN = np.zeros((2, 6), dtype=np.float64)
...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] INV(FLT64 [:, ::1] A):
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] invA = np.zeros((2, 2), dtype=np.float64)
...
@wraparound(False)
@nonecheck(False)
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] JACOBIANO(FLT64 r0, FLT64 r1, FLT64 [:, ::1] element_coord):  
    cdef np.ndarray[FLT64, ndim=2, mode="c"] diffN = MATDIFFN(r0, r1)  
    cdef np.ndarray[FLT64, ndim=2, mode="c"] jac = np.zeros((2, 2), dtype=np.float64)
...
def ShapeFunctions(FLT64 [::1] point_gauss, INT32 nodedof):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] shape_function = MATN(point_gauss[0], point_gauss[1])
    cdef np.ndarray[FLT64, ndim=2, mode="c"] matN = np.zeros((nodedof, 6 * nodedof), dtype=np.float64) 
...
def DiffShapeFuntion(FLT64 [::1] point_gauss, INT32 nodedof):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] diff_shape_function = MATDIFFN(point_gauss[0], point_gauss[1])
    cdef np.ndarray[FLT64, ndim=2, mode="c"] matdiffN = np.zeros((2 * nodedof, 6 * nodedof), dtype=np.float64) 
    cdef Py_ssize_t block, dof
    for block in range(6):
        for dof in range(nodedof):
...
def Jacobian(FLT64 [::1] point_gauss, FLT64 [:, ::1] element_coord):  
    return JACOBIANO(point_gauss[0], point_gauss[1], element_coord)
    
@boundscheck(False)
@wraparound(False)
def invJacobi(FLT64 [::1] point_gauss, FLT64 [:, ::1] element_coord, INT32 nodedof):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] Jac = JACOBIANO(point_gauss[0], point_gauss[1], element_coord)
...
def compute_VOL(    
    ...
    cdef np.ndarray[FLT64, ndim=1, mode="c"] current_pt = np.zeros(2, dtype=np.float64)
...
def StifLinear(
    ...
    cdef np.ndarray[FLT64, ndim=1, mode="c"] current_pt_view = np.zeros(2, dtype=np.float64)
    cdef np.ndarray[FLT64, ndim=2, mode="c"] B
    ...
    for ip in range(intgauss):
        for jp in range(intgauss):
            ...
            current_pt_view[1] = pt_gauss_nodes[jp]
            
            detJ = detJacobi(current_pt_view, element_coord)
            diffN = DiffShapeFuntion(current_pt_view, nodedof)
...
def MassLinear(
    ...
    for ip in range(intgauss):
        for jp in range(intgauss):
            ...
            detJ = detJacobi(current_pt_view, element_coord)
            N = ShapeFunctions(current_pt_view, nodedof)  
...
def NodeList(INT32 [:, ::1] inci, INT32 element_number):
    cdef np.ndarray[INT32, ndim=1, mode="c"] node_list = np.zeros(6, dtype=np.int32)
...
def NodeCoord(FLT64 [:, ::1] coord, INT32 [::1] node_list):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] element_coord = np.zeros((6, 2), dtype=np.float64)
