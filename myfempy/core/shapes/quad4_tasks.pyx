...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATN(FLT64 r0, FLT64 r1):
    ...
    N[0, 0] = 0.25 * (1.0 - r0) * (1.0 - r1)
    N[0, 1] = 0.25 * (1.0 + r0) * (1.0 - r1)
    N[0, 2] = 0.25 * (1.0 + r0) * (1.0 + r1)
    N[0, 3] = 0.25 * (1.0 - r0) * (1.0 + r1)
    return N
...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATDIFFN(FLT64 r0, FLT64 r1):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] dN = np.zeros((2, 4), dtype=np.float64)
    dN[0, 0] = 0.25 * (-1.0 + r1)
    dN[0, 1] = 0.25 * (1.0 - r1)
    dN[0, 2] = 0.25 * (1.0 + r1)
    dN[0, 3] = 0.25 * (-1.0 - r1)
...
def DiffShapeFuntion(FLT64 [::1] point_gauss, INT32 nodedof):
    ...
    for block in range(4):
        for dof in range(nodedof):
            matdiffN[dof * 2, block * nodedof + dof] = diff_shape_function[0, block]
            matdiffN[dof * 2 + 1, block * nodedof + dof] = diff_shape_function[1, block]
    return matdiffN
...
@boundscheck(False)
@wraparound(False)
def NodeList(INT32 [:, ::1] inci, int element_number):
