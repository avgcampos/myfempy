...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATN(FLT64 r0, FLT64 r1):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] N = np.zeros((1, 3), dtype=np.float64)
    N[0, 0] = 1.0 - r0 - r1
    N[0, 1] = r0
    N[0, 2] = r1
    return N
...
cdef inline np.ndarray[FLT64, ndim=2, mode="c"] MATDIFFN(FLT64 r0, FLT64 r1):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] dN = np.zeros((2, 3), dtype=np.float64)
    dN[0, 0] = -1.0
    dN[0, 1] = 1.0
    dN[0, 2] = 0.0
    dN[1, 0] = -1.0
...
def ShapeFunctions(FLT64 [::1] point_gauss, INT32 nodedof):
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] matN = np.zeros((nodedof, 3 * nodedof), dtype=np.float64) 
...
def DiffShapeFuntion(FLT64 [::1] point_gauss, INT32 nodedof):
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] matdiffN = np.zeros((2 * nodedof, 3 * nodedof), dtype=np.float64) 
    cdef Py_ssize_t block, dof
    for block in range(3):
        for dof in range(nodedof):
...
def StifLinear(
    ...
    for ip in range(intgauss):
        for jp in range(intgauss):
            ...
            # 2. Multiplicação e acumulação K_elem_mat += B^T . CB * scale
...
def NodeCoord(FLT64 [:, ::1] coord, INT32 [::1] node_list):
    cdef np.ndarray[FLT64, ndim=2, mode="c"] element_coord = np.zeros((3, 2), dtype=np.float64)
