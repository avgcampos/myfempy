...
@wraparound(False)         
@nonecheck(False) 
cdef inline void MATN(FLT64 r0, FLT64 r1, FLT64 N[3]) noexcept nogil:
    N[0] = 1.0 - r0 - r1
    N[1] = r0
    N[2] = r1
...
@wraparound(False)         
@nonecheck(False) 
cdef inline void MATDIFFN(FLT64 r0, FLT64 r1, FLT64 dN[2][3]) noexcept nogil:
    dN[0][0] = -1.0
    dN[0][1] = 1.0
    dN[0][2] = 0.0
    dN[1][0] = -1.0
...
def StifLinear(
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] diffN = np.zeros((2 * nodedof, 3 * nodedof), dtype=np.float64)
...
def MassLinear(
    ...
    cdef np.ndarray[FLT64, ndim=2, mode="c"] N = np.zeros((nodedof, 3 * nodedof), dtype=np.float64)
